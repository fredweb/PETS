...
namespace XNuvem.Environment.Extensions
{
    public static class ExtensionLoader
    {
        ...
        public static void Load() {
            ...
            foreach (var binPath in binModules) {
                CopyFilesToProbingFolder(binPath);
            }
        ...
        internal static void CopyFileIntoProbing(FileInfo file) {
            ...
            bool shoudCopy = !FileExistsOnBinFolder(file.Name)
                && (!File.Exists(probingFileName) || file.LastWriteTimeUtc > File.GetLastWriteTimeUtc(probingFileName));
            if (shoudCopy) {
                File.Copy(file.FullName, probingFileName, true);
            }
        }
