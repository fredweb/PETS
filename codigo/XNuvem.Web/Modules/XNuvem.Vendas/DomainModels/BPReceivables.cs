...
SELECT
	DocType, RomaneioEntry, DocNum, DocDate, DueDate, CardCode, CardName, SlpCode, SlpName, DocTotal, DocStatus, Comments
FROM (
...
) _DataTable WHERE CardCode = @CardCode ORDER BY DueDate ASC
";

        /// <summary>
        /// Cheques devolvidos
        /// Variáveis: @SlpCode, @CardCode
        /// </summary>
        public const string SqlChequesDevolvidos = @"
SELECT
*
FROM
(
	SELECT DISTINCT  TC1.U_Status DocType, NULL RomaneioEntry,  T0.CheckNum DocNum, T0.RcptDate DocDate, T0.CheckDate DueDate, T1.CardCode, T1.CardName, T2.SlpCode, T2.SlpName, T0.CheckSum DocTotal,
			CASE
						WHEN TC1.U_Status IS NULL  THEN
									'A COMPENSAR'
						ELSE
	...
	LEFT JOIN [@PRV_CH_OCHH] TC1  ON TC1.U_CheckKey = T0.CheckKey
	WHERE T0.Deposited = 'N' AND T0.Canceled = 'N'
) Checks WHERE DocType IN ('5', '6', '7', '8') AND CardCode = @CardCode AND (SlpCode = @SlpCode OR @SlpCode = -1)
";
        ...
        /// Variáveis: @SlpCode, @CardCode
        /// </summary>
        public const string SqlTodosChequesDevolvidos = @"
SELECT
*
FROM
(
	...
	LEFT JOIN [@PRV_CH_OCHH] TC1  ON TC1.U_CheckKey = T0.CheckKey
	WHERE T0.Deposited = 'N' AND T0.Canceled = 'N'
) Checks WHERE DocType IN ('5', '6', '7', '8') AND (SlpCode = {0} OR {0} = -1)
";

        public static string GetSqlTodosChequesDevolvidos(int slpCode) {
            return string.Format(SqlTodosChequesDevolvidos, slpCode);
        }
        ...
        public int RomaneioEntry { get; set; }
