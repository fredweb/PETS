...
$(function () {
    ...
    var setBusinessPartnerCard = function (c) {
    ...
    var initItemsSection = function () {
        ...
        $('[data-xn-toggle="collapse"]').click(function (e) {
            e.preventDefault();
            $(this).parents(".box").find('[data-xn-btn-hide]').click();
        });
    ...
    $("#OrderForm").submit(function (e) {
        ...
        $.ajax({
            ...
            if (data.IsError) {
                ...
                // resetOrderForm(); ?? in edit mode dont reset
                if (data.Messages) {
    ...
    var setBalanceLimit = function (f) {
    ...
    var checkBalanceLimit = function (cardCode) {
        ...
        $.ajax({
            ...
            setBalanceLimit(currentCreditBalance);
        }).fail(function (xhr) {
    ...
    var initFormData = function () {
        // init form data
        setBusinessPartnerCard(currentBusinessPartner);
        setBalanceLimit(currentCreditBalance);
        $("#CardCode").append('<option value="' + currentBusinessPartner.id + '" selected>' + currentBusinessPartner.text + '</option>').trigger('change');
        $("#RotaCode").append('<option value="' + currentRota.id + '" selected>' + currentRota.text + '</option>').trigger('change');
        $("#GroupNum").append('<option value="' + currentGroupNum.id + '" selected>' + currentGroupNum.text + '</option>').trigger('change');
        $("#PeyMethod").append('<option value="' + currentPeyMethod.id + '" selected>' + currentPeyMethod.text + '</option>').trigger('change');
        initItemsSection();
        refreshTotalAll();

        if ($("#DocEntry").val() !== "0") {
            $("#SaveButton").prop("disabled", true);
            $("#SaveInfo").html("Este pedido não pode ser salvo pois ele já foi confirmado no SAP.")
        };
    }
